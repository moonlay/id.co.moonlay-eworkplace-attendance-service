...
namespace EWorkplaceAbsensiService.WebApi.Controllers
{
    ...
    public class TaskManangementController : Controller
    {
        ...
        [HttpGet("/v{version:apiVersion}/TaskProject/{projectid}/{empid}", Name = "get task by project and employee")]
        public async Task<ActionResult> Get(int projectid,int empid,int page)
        {
            try
            {
                VerifyUser();
                var employee = _taskService.GetTaskByProjectAndEmp(projectid,empid);
                return Ok(employee);
            }
            catch (Exception e)
            {
